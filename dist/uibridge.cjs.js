...
`)}),t}generateOpenAPI(){let e=this.registry.getAll(),t={openapi:"3.0.0",info:{title:"UIBridge API",description:"In-app automation framework for web applications",version:this.version,contact:{name:"UIBridge Team"}},servers:[{url:"http://localhost:3000",description:"Local development server"}],paths:{},components:{schemas:{}}};return e.forEach(n=>{let i=`/commands/${n.name}`;t.paths[i]={post:{summary:n.description,description:n.description,requestBody:{required:!0,content:{"application/json":{schema:{type:"object",properties:this._generateJSONSchema(n.parameters),required:n.parameters.filter(s=>s.required).map(s=>s.name)}}}},responses:{200:{description:"Command executed successfully",content:{"application/json":{schema:{type:"object",properties:{success:{type:"boolean"},result:{type:"object"},timestamp:{type:"string",format:"date-time"}}}}}},400:{description:"Invalid parameters"},500:{description:"Command execution failed"}}}}}),t}getStatistics(){let e=this.registry.getAll();return{totalCommands:e.length,commandNames:e.map(t=>t.name),totalParameters:e.reduce((t,n)=>t+n.parameters.length,0),requiredParameters:e.reduce((t,n)=>t+n.parameters.filter(i=>i.required).length,0),optionalParameters:e.reduce((t,n)=>t+n.parameters.filter(i=>!i.required).length,0),commandsWithExamples:e.filter(t=>t.examples&&t.examples.length>0).length,averageParametersPerCommand:Math.round(e.reduce((t,n)=>t+n.parameters.length,0)/e.length*100)/100,lastGenerated:new Date().toISOString()}}_capitalize(e){return e.charAt(0).toUpperCase()+e.slice(1)}_mapTypeToTS(e){return{string:"string",number:"number",boolean:"boolean",object:"object",array:"any[]",Selector:"string | object",ClickOptions:"object",ScreenshotOptions:"object"}[e]||"any"}_generateJSONSchema(e){let t={};return e.forEach(n=>{t[n.name]={type:this._mapTypeToJSONSchema(n.type),description:n.description},n.default!==void 0&&(t[n.name].default=n.default)}),t}_mapTypeToJSONSchema(e){return{string:"string",number:"number",boolean:"boolean",object:"object",array:"array",Selector:"string",ClickOptions:"object",ScreenshotOptions:"object"}[e]||"string"}};var f=class{constructor(e={}){this.options={position:"bottom-right",serverUrl:"http://localhost:3002",autoConnect:!0,minimized:!1,showScreenshots:!0,maxLogEntries:50,...e},this.isConnected=!1,this.commandHistory=[],this.element=null,this.isMinimized=this.options.minimized,this.wsConnection=null,this.init()}init(){this.createPanel(),this.attachStyles(),this.options.autoConnect&&this.connectToServer()}createPanel(){this.element=document.createElement("div"),this.element.className="uibridge-debug-panel",this.element.innerHTML=this.getHTML(),this.element.style.position="fixed",this.element.style.zIndex="999999",this.setPosition(),this.attachEventListeners(),document.body.appendChild(this.element),this.updateConnectionStatus()}getHTML(){return`
    ...
    `,document.head.appendChild(e)}setPosition(){this.element.className=`uibridge-debug-panel ${this.options.position}`}attachEventListeners(){this.element.querySelector(".minimize-btn").addEventListener("click",()=>this.toggleMinimize()),this.element.querySelector(".close-btn").addEventListener("click",()=>this.destroy()),this.element.querySelector(".connect-btn").addEventListener("click",()=>this.toggleConnection()),this.element.querySelector(".server-url").addEventListener("change",s=>{this.options.serverUrl=s.target.value}),this.makeDraggable()}makeDraggable(){let e=this.element.querySelector(".debug-panel-header"),t=!1,n=0,i=0,s=0,r=0;e.addEventListener("mousedown",a=>{t=!0,s=a.clientX-n,r=a.clientY-i}),document.addEventListener("mousemove",a=>{t&&(a.preventDefault(),n=a.clientX-s,i=a.clientY-r,this.element.style.transform=`translate(${n}px, ${i}px)`)}),document.addEventListener("mouseup",()=>{t=!1})}toggleMinimize(){this.isMinimized=!this.isMinimized;let e=this.element.querySelector(".debug-panel-content"),t=this.element.querySelector(".minimize-btn");e.style.display=this.isMinimized?"none":"block",t.textContent=this.isMinimized?"\u2B06\uFE0F":"\u2B07\uFE0F",t.title=this.isMinimized?"Expand":"Minimize"}async connectToServer(){try{if((await fetch(`${this.options.serverUrl}/health`)).ok)this.isConnected=!0,this.logActivity("Connected to UIBridge server","success"),this.startPolling();else throw new Error("Server not responding")}catch(e){this.isConnected=!1,this.logActivity(`Connection failed: ${e.message}`,"error")}this.updateConnectionStatus()}disconnectFromServer(){this.isConnected=!1,this.pollingInterval&&clearInterval(this.pollingInterval),this.logActivity("Disconnected from server","info"),this.updateConnectionStatus()}toggleConnection(){this.isConnected?this.disconnectFromServer():this.connectToServer()}startPolling(){this.pollingInterval=setInterval(async()=>{try{let e=await fetch(`${this.options.serverUrl}/activity`);if(e.ok){let t=await e.json();t.commands&&t.commands.length>0&&this.handleNewCommands(t.commands)}}catch{}},500)}handleNewCommands(e){e.forEach(t=>{!this.commandHistory.find(i=>i.id===t.id)&&(this.logActivity(`${t.command.toUpperCase()}: ${t.selector||"page"}`,t.success?"success":"error"),t.screenshot&&this.updateScreenshot(t.screenshot),this.commandHistory.push(t))}),this.updateStats()}updateConnectionStatus(){let e=this.element.querySelector(".connection-status"),t=this.element.querySelector(".connect-btn");e.textContent=this.isConnected?"\u{1F7E2}":"\u{1F534}",e.className=`connection-status ${this.isConnected?"connected":"disconnected"}`,t.textContent=this.isConnected?"Disconnect":"Connect",t.className=this.isConnected?"connect-btn disconnect":"connect-btn"}logActivity(e,t="info"){let n=new Date().toLocaleTimeString(),i={timestamp:n,message:e,type:t,time:Date.now()},s=this.element.querySelector(".command-log"),r=document.createElement("div");for(r.className=`log-entry ${t}`,r.innerHTML=`
      <span class="log-timestamp">${n}</span>
      ${e}
    `,s.appendChild(r),s.scrollTop=s.scrollHeight;s.children.length>this.options.maxLogEntries;)s.removeChild(s.firstChild)}updateScreenshot(e){if(!this.options.showScreenshots)return;let t=this.element.querySelector(".screenshot-preview");t.innerHTML=`<img src="${e}" alt="Latest screenshot" />`}updateStats(){let e=this.commandHistory.length,t=this.commandHistory.filter(i=>i.success).length,n=this.commandHistory.filter(i=>!i.success).length;this.element.querySelector("#command-count").textContent=e,this.element.querySelector("#success-count").textContent=t,this.element.querySelector("#error-count").textContent=n}destroy(){this.pollingInterval&&clearInterval(this.pollingInterval),this.element&&this.element.parentNode&&this.element.parentNode.removeChild(this.element);let e=document.getElementById("uibridge-debug-styles");e&&e.remove()}show(){this.element.style.display="block"}hide(){this.element.style.display="none"}setDebugPanelPosition(e){this.options.position=e,this.setPosition()}};var y={name:"click",description:"Clicks on an element using synthetic mouse events",examples:["execute('click', '#submit-button')","execute('click', { text: 'Submit' })","execute('click', { testId: 'login-btn' })","execute('click', '#button', { position: 'center', clickCount: 2 })"],parameters:[{name:"selector",type:"Selector",required:!0,description:"Element to click (string, CSS selector, or selector object)"},{name:"options",type:"ClickOptions",required:!1,description:"Click options: { force?, position?, button?, clickCount?, delay? }"}],async execute(o,e,t={}){let n=o.findElement(e);if(!n)throw new Error(`Element not found: ${JSON.stringify(e)}`);let i={force:!1,position:"center",button:"left",clickCount:1,delay:0,scrollIntoView:!0,...t};if(o._log(`Clicking element: ${o.selectorEngine.getElementInfo(n)?.tag||"unknown"}`),i.scrollIntoView&&(n.scrollIntoView({behavior:"smooth",block:"center"}),await new Promise(l=>setTimeout(l,100))),!i.force&&!o.selectorEngine.isVisible(n))throw new Error("Element is not visible. Use { force: true } to click anyway.");if(!i.force&&!this._isElementActionable(n))throw new Error("Element is covered by another element. Use { force: true } to click anyway.");let s=n.getBoundingClientRect(),r=this._calculatePosition(s,i.position),a={bubbles:!0,cancelable:!0,view:window,clientX:r.x,clientY:r.y,button:this._getButtonCode(i.button),buttons:this._getButtonsCode(i.button),detail:i.clickCount};try{n.dispatchEvent(new MouseEvent("mouseover",a)),n.dispatchEvent(new MouseEvent("mouseenter",a)),n.dispatchEvent(new MouseEvent("mousedown",a)),i.delay>0&&await new Promise(l=>setTimeout(l,i.delay)),n.dispatchEvent(new MouseEvent("mouseup",a));for(let l=0;l<i.clickCount;l++)l>0&&await new Promise(c=>setTimeout(c,50)),n.dispatchEvent(new MouseEvent("click",{...a,detail:l+1}));o.selectorEngine._isFocusable(n)&&n.focus(),await this._handleSpecialElements(n,i)}catch(l){throw new Error(`Failed to click element: ${l.message}`)}return{success:!0,element:o.selectorEngine.getElementInfo(n),position:r,timestamp:new Date().toISOString()}},_isElementActionable(o){let e=o.getBoundingClientRect(),t=e.left+e.width/2,n=e.top+e.height/2,i=document.elementFromPoint(t,n);return o===i||o.contains(i)},_calculatePosition(o,e){let t={center:{x:o.left+o.width/2,y:o.top+o.height/2},topLeft:{x:o.left+1,y:o.top+1},topRight:{x:o.right-1,y:o.top+1},bottomLeft:{x:o.left+1,y:o.bottom-1},bottomRight:{x:o.right-1,y:o.bottom-1},topCenter:{x:o.left+o.width/2,y:o.top+1},bottomCenter:{x:o.left+o.width/2,y:o.bottom-1},leftCenter:{x:o.left+1,y:o.top+o.height/2},rightCenter:{x:o.right-1,y:o.top+o.height/2}};return t[e]||t.center},_getButtonCode(o){return{left:0,middle:1,right:2}[o]||0},_getButtonsCode(o){return{left:1,middle:4,right:2}[o]||1},async _handleSpecialElements(o,e){let t=o.tagName.toLowerCase(),n=o.type?.toLowerCase();t==="button"&&o.type==="submit"&&o.closest("form")||t==="input"&&(n==="checkbox"||n==="radio")||(t==="select"&&setTimeout(()=>{o.dispatchEvent(new Event("change",{bubbles:!0}))},10),t==="a"&&o.href)}};var b={name:"screenshot",description:"Takes a screenshot of the page or a specific element",examples:["execute('screenshot')","execute('screenshot', { format: 'png', quality: 0.9 })","execute('screenshot', { selector: '#main-content' })","execute('screenshot', { fullPage: true, saveConfig: { autoSave: true, folder: 'tests' } })"],parameters:[{name:"options",type:"ScreenshotOptions",required:!1,description:"Screenshot options: { selector?, format?, quality?, fullPage?, saveConfig? }"}],async execute(o,e={}){console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Starting screenshot command execution"),console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Raw options received:",JSON.stringify(e,null,2)),console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Bridge config:",JSON.stringify(o.config.defaultScreenshotConfig,null,2));let t={selector:null,format:"png",quality:.92,fullPage:!1,excludeSelectors:[],backgroundColor:null,scale:window.devicePixelRatio||1,saveConfig:{...o.config.defaultScreenshotConfig,...e.saveConfig},...e};console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Final processed options:",JSON.stringify(t,null,2)),o._log("Taking screenshot with options:",t);let n=document.body;if(t.selector&&(n=o.findElement(t.selector),!n))throw new Error(`Element not found for screenshot: ${JSON.stringify(t.selector)}`);try{console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Target element:",n?.tagName,n?.id,n?.className),console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Target element dimensions:",{width:n?.offsetWidth,height:n?.offsetHeight,scrollWidth:n?.scrollWidth,scrollHeight:n?.scrollHeight}),console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Loading html2canvas...");try{await this._ensureHtml2Canvas(),console.log("\u{1F5BC}\uFE0F [SCREENSHOT] html2canvas loaded:",!!window.html2canvas)}catch(u){throw console.error("\u{1F5BC}\uFE0F [SCREENSHOT] Failed to load html2canvas:",u.message),new Error(`Failed to load html2canvas library: ${u.message}. Please ensure you have internet connectivity or consider using a different screenshot method.`)}let i=this._hideElements(t.excludeSelectors),s={useCORS:!0,allowTaint:!1,backgroundColor:t.backgroundColor,scale:t.scale,logging:!0,width:t.fullPage?document.documentElement.scrollWidth:void 0,height:t.fullPage?document.documentElement.scrollHeight:void 0,windowWidth:t.fullPage?document.documentElement.scrollWidth:void 0,windowHeight:t.fullPage?document.documentElement.scrollHeight:void 0,x:t.fullPage?0:void 0,y:t.fullPage?0:void 0,foreignObjectRendering:!0,imageTimeout:15e3,removeContainer:!0};console.log("\u{1F5BC}\uFE0F [SCREENSHOT] html2canvas options:",JSON.stringify(s,null,2)),console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Starting html2canvas capture...");let r=await Promise.race([window.html2canvas(n,s),new Promise((u,v)=>setTimeout(()=>v(new Error("Screenshot capture timed out after 30 seconds")),3e4))]);console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Canvas created:",{width:r.width,height:r.height,hasData:r.getContext("2d").getImageData(0,0,1,1).data.some(u=>u!==0)}),this._restoreElements(i);let a=`image/${t.format}`;console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Converting to format:",a,"quality:",t.quality);let l=r.toDataURL(a,t.quality);console.log("\u{1F5BC}\uFE0F [SCREENSHOT] DataURL created, length:",l.length,"starts with:",l.substring(0,50));let c=this._generateFileName(t);console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Generated filename:",c),t.saveConfig.autoSave?(console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Auto-save enabled, saving..."),await this._saveScreenshot(l,c,t.saveConfig),console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Save completed")):console.log("\u{1F5BC}\uFE0F [SCREENSHOT] Auto-save disabled");let d={success:!0,dataUrl:l,width:r.width,height:r.height,format:t.format,fileName:c,filePath:t.saveConfig.folder?`${t.saveConfig.folder}/${c}`:c,size:Math.round(l.length*.75),timestamp:new Date().toISOString(),saveConfig:t.saveConfig};return t.selector&&(d.element=o.selectorEngine.getElementInfo(n),t.saveConfig.includeMetadata&&(d.metadata={selector:t.selector,element:d.element,viewport:{width:window.innerWidth,height:window.innerHeight},userAgent:navigator.userAgent,timestamp:d.timestamp})),o._log(`Screenshot captured: ${d.width}x${d.height}, ${d.size} bytes, saved as: ${d.filePath}`),d}catch(i){throw new Error(`Failed to take screenshot: ${i.message}`)}},_generateFileName(o){let e=o.saveConfig;if(e.customName)return this._ensureExtension(e.customName,o.format);let t=e.prefix||"screenshot";if(e.includeMetadata){if(o.selector){let n=typeof o.selector=="string"?o.selector.replace(/[#.]/g,"").substring(0,20):"element";t+=`_${n}`}o.fullPage&&(t+="_fullpage"),t+=`_${o.width||"auto"}x${o.height||"auto"}`}if(e.timestamp){let n=new Date().toISOString().replace(/[:.]/g,"-").replace("T","_").substring(0,19);t+=`_${n}`}return this._ensureExtension(t,o.format)},_ensureExtension(o,e){let t=e==="jpeg"?"jpg":e;return o.toLowerCase().endsWith(`.${t}`)?o:`${o}.${t}`},async _saveScreenshot(o,e,t){try{await this._downloadImage(o,e),t.serverEndpoint&&await this._saveToServer(o,e,t),t.persistInBrowser&&await this._saveToIndexedDB(o,e,t)}catch(n){throw console.warn("Failed to save screenshot:",n),new Error(`Screenshot save failed: ${n.message}`)}},async _saveToServer(o,e,t){if(t.serverEndpoint)try{let n=await fetch(t.serverEndpoint,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({fileName:e,folder:t.folder,dataUrl:o,timestamp:new Date().toISOString()})});if(!n.ok)throw new Error(`Server save failed: ${n.status} ${n.statusText}`);console.log(`Screenshot saved to server: ${e}`)}catch(n){throw console.error("Server save error:",n),n}},async _saveToIndexedDB(o,e,t){return new Promise((n,i)=>{let s="UIBridgeScreenshots",r="screenshots",a=indexedDB.open(s,1);a.onerror=()=>i(new Error("IndexedDB open failed")),a.onupgradeneeded=l=>{let c=l.target.result;if(!c.objectStoreNames.contains(r)){let d=c.createObjectStore(r,{keyPath:"id",autoIncrement:!0});d.createIndex("fileName","fileName",{unique:!1}),d.createIndex("timestamp","timestamp",{unique:!1})}},a.onsuccess=l=>{let u=l.target.result.transaction([r],"readwrite").objectStore(r),v={fileName:e,folder:t.folder,dataUrl:o,timestamp:new Date().toISOString(),size:Math.round(o.length*.75)},S=u.add(v);S.onsuccess=()=>{console.log(`Screenshot saved to IndexedDB: ${e}`),n()},S.onerror=()=>{i(new Error("IndexedDB save failed"))}}})},async _ensureHtml2Canvas(){if(!window.html2canvas)return new Promise((o,e)=>{let t=["https://unpkg.com/html2canvas@1.4.1/dist/html2canvas.min.js","https://cdn.jsdelivr.net/npm/html2canvas@1.4.1/dist/html2canvas.min.js","https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js","https://unpkg.com/html2canvas@latest/dist/html2canvas.min.js"],n=0,i=()=>{if(n>=t.length){e(new Error("Failed to load html2canvas from all CDN sources. Please check your internet connection or consider using a different screenshot method."));return}let s=document.createElement("script");s.src=t[n],s.crossOrigin="anonymous",s.async=!0,s.src.includes("cdnjs.cloudflare.com")&&(s.integrity="sha512-UHwkWYKZYKtkT6k7iF4FITLJAVkI/J1iOtLwSbwUXf/R+0P+WFbHFvdPRyZOmJIa3V1p8Yj8FxkgnyFb1m4qw=="),s.onload=()=>{setTimeout(()=>{if(window.html2canvas&&typeof window.html2canvas=="function")try{let c=document.createElement("div");c.style.position="absolute",c.style.left="-9999px",c.style.width="1px",c.style.height="1px",document.body.appendChild(c),window.html2canvas(c,{width:1,height:1}).then(()=>{document.body.removeChild(c),console.log("\u{1F5BC}\uFE0F [SCREENSHOT] html2canvas loaded and validated successfully from:",t[n]),o()}).catch(()=>{document.body.removeChild(c),console.warn("\u{1F5BC}\uFE0F [SCREENSHOT] html2canvas loaded but failed validation test, trying next source..."),n++,i()})}catch{console.warn("\u{1F5BC}\uFE0F [SCREENSHOT] html2canvas loaded but failed basic test, trying next source..."),n++,i()}else console.warn("\u{1F5BC}\uFE0F [SCREENSHOT] html2canvas loaded but not functional, trying next source..."),n++,i()},200)},s.onerror=()=>{console.warn("\u{1F5BC}\uFE0F [SCREENSHOT] Failed to load html2canvas from:",t[n]),n++,i()};let r=s.onload,a=setTimeout(()=>{console.warn("\u{1F5BC}\uFE0F [SCREENSHOT] Timeout loading html2canvas from:",t[n]),n++,i()},1e4);s.onload=()=>{clearTimeout(a),r()};let l=document.querySelector(`script[src="${t[n]}"]`);if(l){window.html2canvas&&typeof window.html2canvas=="function"?o():(l.onload=s.onload,l.onerror=s.onerror);return}document.head.appendChild(s)};i()})},_hideElements(o){let e=[];for(let t of o)try{let n=document.querySelectorAll(t);for(let i of n){let s=i.style.display;i.style.display="none",e.push({element:i,originalDisplay:s})}}catch(n){console.warn(`Invalid selector for hiding: ${t}`,n)}return e},_restoreElements(o){for(let{element:e,originalDisplay:t}of o)e.style.display=t},_downloadImage(o,e){try{let t=document.createElement("a");t.download=e,t.href=o,document.body.appendChild(t),t.click(),document.body.removeChild(t)}catch(t){console.warn("Failed to auto-download screenshot:",t)}}};var C={name:"help",description:"Get help information about UIBridge commands and usage patterns for AI automation",examples:["execute('help')","execute('help', 'click')","execute('help', 'screenshot')","execute('--help')"],parameters:[{name:"commandName",type:"string",required:!1,description:"Specific command to get help for (optional)"}],async execute(o,e=null){return o.getHelp(e)}};var m=class o{constructor(e={}){this.config={debug:!1,allowedOrigins:["*"],commands:["click","screenshot","help"],generateCDI:!0,enableHttpDiscovery:!1,autoInit:!0,version:"1.3.2",enableRemoteControl:!1,serverUrl:"http://localhost:3002",pollInterval:500,autoStartPolling:!0,showDebugPanel:!1,debugPanelOptions:{position:"bottom-right",minimized:!1,showScreenshots:!0,autoConnect:!0},defaultScreenshotConfig:{autoSave:!1,folder:"uibridge-screenshots",prefix:"screenshot",timestamp:!0,includeMetadata:!1,persistInBrowser:!1,serverEndpoint:null,...e.defaultScreenshotConfig},...e},this.registry=new h,this.selectorEngine=new g,this.cdiGenerator=null,this.debugPanel=null,this._isInitialized=!1,this._initStartTime=null,this._commandHistory=[],this._isPolling=!1,this._pollTimeoutId=null,this.config.autoInit&&typeof window<"u"&&typeof document<"u"&&(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>this.init()):setTimeout(()=>this.init(),0))}async init(){if(this._isInitialized){this._log("UIBridge already initialized");return}this._initStartTime=typeof performance<"u"?performance.now():Date.now(),this._log("Initializing UIBridge...",this.config);try{await this._registerCoreCommands(),this.cdiGenerator=new p(this.registry),this._setupDiscovery(),this._setupGlobalAPI(),this._isInitialized=!0;let e=(typeof performance<"u"?performance.now():Date.now())-this._initStartTime;this._log(`UIBridge initialized successfully in ${e.toFixed(2)}ms`,{commands:this.registry.getNames(),version:this.config.version}),this.config.generateCDI&&this._generateCDI(),this.config.enableRemoteControl&&this.config.autoStartPolling&&this.startRemoteControl(),this.config.showDebugPanel&&typeof window<"u"&&this._initDebugPanel(),this._dispatchEvent("uibridge:initialized",{version:this.config.version,commands:this.registry.getNames(),initTime:e,remoteControlEnabled:this.config.enableRemoteControl,debugPanelEnabled:this.config.showDebugPanel})}catch(e){throw this._log("Failed to initialize UIBridge:",e),new Error(`UIBridge initialization failed: ${e.message}`)}}async execute(e,...t){if(e==="help"||e==="--help")return this.getHelp(t[0]);if(!this._isInitialized)throw new Error("UIBridge not initialized. Call init() first.");let n=this.registry.get(e);if(!n){let r=this.registry.getAll().map(a=>a.name).join(", ");throw new Error(`Unknown command: ${e}. Available commands: ${r}. Use 'help' for detailed information.`)}let i=typeof performance<"u"?performance.now():Date.now(),s=this._generateExecutionId();this._log(`Executing command: ${e}`,{args:t,executionId:s});try{let r={id:s,command:e,args:t,startTime:new Date().toISOString(),status:"running"};this._commandHistory.push(r);let a=await n.execute(this,...t),c=(typeof performance<"u"?performance.now():Date.now())-i;r.status="completed",r.duration=c,r.result=a,r.endTime=new Date().toISOString(),this._log(`Command completed: ${e} (${c.toFixed(2)}ms)`,a),this._dispatchEvent("uibridge:command",{command:e,args:t,result:a,duration:c,executionId:s});let d={...a,command:e,duration:c,timestamp:new Date().toISOString()};return this._addToHistory({command:e,args:t,result:d,duration:c,timestamp:new Date().toISOString(),status:"completed"}),d}catch(r){let l=(typeof performance<"u"?performance.now():Date.now())-i,c=this._commandHistory[this._commandHistory.length-1];throw c&&c.id===s&&(c.status="failed",c.error=r.message,c.duration=l,c.endTime=new Date().toISOString()),this._log(`Command failed: ${e} (${l.toFixed(2)}ms)`,r),this._dispatchEvent("uibridge:error",{command:e,args:t,error:r.message,duration:l,executionId:s}),this._addToHistory({command:e,args:t,error:r.message,duration:l,timestamp:new Date().toISOString(),status:"failed"}),r}}findElement(e){return this.selectorEngine.find(e)}findElements(e){return this.selectorEngine.findAll(e)}discover(){return this.registry.getAll().map(e=>({name:e.name,description:e.description,parameters:e.parameters,examples:e.examples||[]}))}getHistory(e=50){return this._commandHistory.slice(-e)}clearHistory(){this._commandHistory=[],this._log("Command history cleared")}getStatus(){return{initialized:this._isInitialized,version:this.config.version,commands:this.registry.getNames(),commandCount:this.registry.size(),historyLength:this._commandHistory.length,config:{...this.config},uptime:this._initStartTime?(typeof performance<"u"?performance.now():Date.now())-this._initStartTime:0}}registerCommand(e,t){this.registry.register(e,t),this._log(`Custom command registered: ${e}`),this.cdiGenerator&&this._generateCDI()}unregisterCommand(e){let t=this.registry.unregister(e);return t&&(this._log(`Command unregistered: ${e}`),this.cdiGenerator&&this._generateCDI()),t}configureScreenshots(e){this.config.defaultScreenshotConfig={...this.config.defaultScreenshotConfig,...e},this._log("Screenshot configuration updated:",this.config.defaultScreenshotConfig)}getScreenshotConfig(){return{...this.config.defaultScreenshotConfig}}async _registerCoreCommands(){let e=[{name:"click",implementation:y},{name:"screenshot",implementation:b},{name:"help",implementation:C}];for(let{name:t,implementation:n}of e)this.config.commands.includes(t)&&(this.registry.register(t,n),this._log(`Registered core command: ${t}`))}_setupDiscovery(){this.config.enableHttpDiscovery&&(window.__uibridge_discovery__=()=>this.discover(),window.__uibridge_status__=()=>this.getStatus(),this._log("HTTP discovery endpoints enabled"))}_setupGlobalAPI(){window.UIBridge=o,window.uibridge=this,window.__uibridge__={execute:this.execute.bind(this),discover:this.discover.bind(this),findElement:this.findElement.bind(this),findElements:this.findElements.bind(this),getStatus:this.getStatus.bind(this),getHistory:this.getHistory.bind(this)},this._log("Global APIs exposed")}_generateCDI(){if(this.cdiGenerator)try{this.config.debug&&(console.log("=== UIBridge Command Discovery Interface ==="),console.log(this.cdiGenerator.generateMarkdown()),console.log("=== End CDI ===")),window.__uibridge_cdi__={markdown:()=>this.cdiGenerator.generateMarkdown(),json:()=>this.cdiGenerator.generateJSON(),save:e=>this.cdiGenerator.saveToFile(e)}}catch(e){this._log("Failed to generate CDI:",e)}}_generateExecutionId(){return`exec_${Date.now()}_${Math.random().toString(36).substr(2,9)}`}_dispatchEvent(e,t){if(typeof window<"u"&&window.dispatchEvent){let n=new CustomEvent(e,{detail:t});window.dispatchEvent(n)}}_log(...e){this.config.debug&&console.log("[UIBridge]",...e)}getHelp(e=null){if(e){let n=this.registry.get(e);if(!n){let i=this.registry.getAll().map(s=>s.name).join(", ");return{error:`Unknown command: ${e}`,availableCommands:i,suggestion:"Use 'help' without arguments to see all commands",aiGuidance:`AI Agents: Use execute('help') to discover available commands or check spelling of '${e}'`}}return{command:n.name,description:n.description,parameters:n.parameters,examples:n.examples||[],usage:this._generateUsage(n),aiTips:this._getAITipsForCommand(n.name)}}let t=this.registry.getAll();return{framework:"UIBridge",version:this.config.version||"1.0.0",description:"In-app automation framework for web applications - designed for AI agent control",aiQuickStart:{step1:"Execute commands using: await uibridge.execute('commandName', ...args)",step2:"Find elements using selectors: CSS, text content, test IDs, XPath",step3:"Handle errors with try/catch blocks",step4:"Use await for all commands as they return promises"},commands:t.map(n=>({name:n.name,description:n.description,parameters:n.parameters.length,usage:this._generateUsage(n),aiUseCase:this._getAIUseCase(n.name)})),automationPatterns:{"Click any button":{pattern:"execute('click', selector)",examples:["execute('click', '#submit')","execute('click', { text: 'Submit' })","execute('click', { testId: 'submit-btn' })"],aiTip:"Try multiple selector strategies if one fails"},"Take screenshots":{pattern:"execute('screenshot', options)",examples:["execute('screenshot', { fullPage: true })","execute('screenshot', { selector: '#content' })","execute('screenshot', { saveConfig: { autoSave: true } })"],aiTip:"Screenshots are useful for verification and debugging"},"Discover available actions":{pattern:"discover() or execute('help')",examples:["const commands = uibridge.discover()","const help = await uibridge.execute('help')"],aiTip:"Use this to understand what actions are available"}},selectorStrategies:{priority:"Try strategies in this order for best results",strategies:{1:{method:"Test ID",syntax:"{ testId: 'element-id' }",reliability:"highest"},2:{method:"CSS ID",syntax:"'#element-id'",reliability:"high"},3:{method:"CSS Class",syntax:"'.class-name'",reliability:"medium"},4:{method:"Text Content",syntax:"{ text: 'Button Text' }",reliability:"medium"},5:{method:"Aria Label",syntax:"{ ariaLabel: 'Button Label' }",reliability:"medium"},6:{method:"XPath",syntax:`{ xpath: '//button[text()="Submit"]' }`,reliability:"advanced"}}},aiBestPractices:["Always use await when executing commands","Wrap commands in try/catch blocks for error handling","Use specific selectors (ID, testId) when possible","Take screenshots to verify actions completed successfully","Use help('commandName') to understand specific command options","Check element existence before interaction","Wait for dynamic content to load before acting"],errorHandling:{"Element not found":{solution:"Try different selector strategies or wait for element to appear",code:"try { await execute('click', '#btn'); } catch(e) { await execute('click', {text: 'Submit'}); }"},"Command failed":{solution:"Check command syntax and parameters",code:"const help = await execute('help', 'click'); console.log(help.usage);"},"Screenshot failed":{solution:"Ensure element is visible and page is loaded",code:"await execute('screenshot', { selector: 'body', fullPage: true });"}},workflowPatterns:{"Form submission":["1. Find and fill input fields","2. Click submit button","3. Take screenshot to verify","Example: execute('click', { text: 'Submit' })"],"Navigation and verification":["1. Take screenshot of current state","2. Click navigation element","3. Wait for page load","4. Take screenshot to verify navigation"],"Content interaction":["1. Find target element using multiple selector strategies","2. Execute action (click, screenshot, etc.)","3. Verify result with screenshot or status check"]},powershellPatterns:{"Basic Click Command":{description:"Execute click action via PowerShell REST API",code:`$params = @{
